<div class="bg--orange">
    <div id="renovacao" class="hold-everybody escola-renovacao__bg">
        <div class="d-flex align-items-center flex-direction__desktop py-5">
            <div class="col-12 col-md-8 text-center">
                <x-common.title class="my-4" title="landing-page.renovacao-school" subtitle="landing-page.renovacao-school__sub"/>
                <div class="container-iframe">
                    <iframe class="responsive-iframe" src="https://www.youtube.com/embed/r0_8IddTHIc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
                <br>
                <a target="_blank" href="https://wa.me/554196310071">
                    <x-common.button-whats />
                </a>
            </div>
        </div>
    </div>
</div>